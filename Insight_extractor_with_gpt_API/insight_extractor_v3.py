...
# argparse 설정
parser = argparse.ArgumentParser(description='Process some quotes.')
parser.add_argument('--part', type=int, choices=[0, 1, 2, 3], required=True, help='Part of the dataset to process (0, 1, 2, 3)')
args = parser.parse_args()

# 범위 설정
start_index = args.part * 2500
end_index = start_index + 2499

# 실행 시간 측정 시작
start_time = time.time()

df = pd.read_csv("/home/jdh251425/2025_prometheus/Dataset/sampled_quotes.csv")
quotes = df['quote'].tolist()[start_index:end_index]
authors = df['author'].tolist()[start_index:end_index]
categories = df['category'].tolist()[start_index:end_index]
...
# 결과를 CSV 파일로 저장
result_df.to_csv(f'quotes_with_insights_part_{args.part}.csv', index=False)
print(f"작업 완료: 'quotes_with_insights_part_{args.part}.csv' 파일에 저장되었습니다.")
